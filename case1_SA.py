...
def compute_aep(x):
    ...
    #else:
        #aep = aep*1.0
    f = open('case1/direct/SA/rand10/records'+str(i+1)+'.txt','w')
    i = i+1
...
optimize_result = dual_annealing(discrete_objective,bounds,no_local_search = True,seed = 100010)#,constraints = [con])
decoded_solution = discrete_objective.decode(optimize_result.x)
decoded_fun = discrete_objective(decoded_solution)
end_time = time.time()
time_running = end_time-start_time

np.savetxt('case1/direct/SA/rand10/results.txt',x_total,delimiter = ',',fmt='%.3f')   
f = open('case1/direct/SA/rand10/summary.txt','w')
f.write('Consuming time is '+str(time_running)+' s\n')
