...
def compute_aep(x):
    ...
    f = open('case3/direct/SA/rand10/records'+str(i+1)+'.txt','w')
    i = i+1
...
for i in range(init.shape[0]):
    init_aep = np.append(init_aep,compute_aep(init[i]))
x0_idx = np.argmax(init_aep)
x0 = init[x0_idx]
...
optimize_result = dual_annealing(discrete_objective,bounds,no_local_search = True,seed = 100010,x0 = x0)
decoded_solution = discrete_objective.decode(optimize_result.x)
decoded_fun = discrete_objective(decoded_solution)
end_time = time.time()
time_running = end_time-start_time

np.savetxt('case3/direct/SA/rand10/results.txt',x_total,delimiter = ',',fmt='%.3f')   
f = open('case3/direct/SA/rand10/summary.txt','w')
f.write('Consuming time is '+str(time_running)+' s\n')
